...
**/
  ...
  // Tracking account.
  define("PC_GA_ACCOUNT", "UA-3107726-1");
  define("MB_GA_ACCOUNT", "MO-3107726-1");
  ...
  function isFeaturePhone() {
    ...
    if (preg_match('/^DoCoMo/', $userAgent) ||
        ...
        preg_match('/^MOT-C980/', $userAgent) ||
        preg_match('/^MOT-V980/', $userAgent)) {
      return true;
    } else {
      return false;
    }
  ...
  function isSmartPhone() {
    ...
    if (preg_match('/^Mozilla[^\(]*\(iPhone/', $userAgent) ||
        preg_match('/^Mozilla[^\(]*\(iPad/', $userAgent) ||
        preg_match('/^Mozilla[^\(]*\(iPod/', $userAgent) ||
        preg_match('/^Mozilla.*Android/', $userAgent)) {
      return true;
    } else {
      return false;
    }
  ...
  function trackPageView() {
    ...
    // pixel.
    $documentReferer = urlencode(@$_SERVER["HTTP_REFERER"]);
    if (empty($documentReferer) && $documentReferer !== "0") {
      $documentReferer = "-";
    } else {
      $documentReferer = urldecode($documentReferer);
    }
    $documentPath = urlencode(@$_SERVER["REQUEST_URI"]);
    ...
    if (isFeaturePhone()) {
      $account = MB_GA_ACCOUNT;
    } else {
      $account = PC_GA_ACCOUNT;
    }
    ...
    // Finally redirect to another url.
    $redirect_url = @$_GET["redirect_url"];
    if (isFeaturePhone() && @$_GET["mb_redirect_url"]) {
      $redirect_url = @$_GET["mb_redirect_url"];
    }
    if (isSmartPhone() && @$_GET["sp_redirect_url"]) {
      $redirect_url = @$_GET["sp_redirect_url"];
    }
