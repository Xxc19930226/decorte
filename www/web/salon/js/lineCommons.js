...
// lineContents内スクロールバー置換え処理
/*if(navigator.userAgent.indexOf("Mac") != -1 ){
	$(function() {
		$('.scroll-pane').jScrollPane({reinitialiseOnImageLoad: true});
		// 20110629 Macでリロード時にスクロールバーが消える問題に対処
	});
} else {
	$(function() {
...
// ラインサイト内リンク処理　　20110628 全頁固有のURLを付与するためindexを複数ページに切り分けたため、この処理は不要
/*$(function() {
	// スキンケアシステムへのダイレクトリンク処理
	var anc = window.location.href.slice(window.location.href.indexOf('#') + 1);
	if(anc == "skincaresystem") {
		$("#lineItems").hide();
		$("a.itemsLink").removeClass("selected");
		$("div#lineMenu a.inline").removeClass("selected");
		$("div#lineMenu dt.listSkinCare a.inline").addClass("selected");
	} else {
		// ラインサイト内リンク処理
		$("#lineItems").hide();
		$("#lineContents ul li:not("+$("div#lineMenu a.selected").attr("href")+")").hide();
		$("div#lineMenu a.inline").click(function() {
			if ($(this).hasClass("selected")) {
				return false;	// スライド表示中のメニュー動作キャンセル
			}
			$("#lineItems").hide();
			$("a.itemsLink").removeClass("selected");
			$("#lineContents").hide();
			$("#lineContents").fadeIn("slow");
			$("div#lineMenu a.inline").removeClass("selected");
			$(this).addClass("selected");
			$("#lineContents li").fadeOut("fast");
			$($(this).attr("href")).fadeIn("slow");
			return false;
		});
	}
	// アイテム一覧読み込みajax切替処理
	$("a.itemsLink").click(function() {
		if ($(this).hasClass("selected")) {
			return false;
		}
		// jScrollPaneのイニシャライズ
		...
		$("a.itemsLink").removeClass("selected");
		$(this).addClass("selected");
		$("#lineItems").hide().load($(this).attr('href'), '', reinitialiseScrollPane).fadeIn("slow");
		return false;
	});
